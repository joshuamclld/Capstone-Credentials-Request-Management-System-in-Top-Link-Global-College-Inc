...
export default function StudentNotificationBell({ onNavigate }) {
    ...
    useEffect(() => {
        ...
        const fetchWithSignal = () => {
            fetch('/student/api/notifications', { credentials: 'same-origin', signal: controller.signal })
                .then(res => res.json())
                .then(data => {
    ...
    const openDropdown = () => {
        if (fetching) return;
        setFetching(true);
        fetch('/student/api/notifications', { credentials: 'same-origin' })
            .then(res => res.json())
            .then(data => {
                setNotifications(data.notifications || []);
                setUnreadCount(data.unread_count || 0);
    ...
    return (
        <div ref={bellRef} className="relative">
            <button
                ...
                className="p-2 rounded-lg text-on-surface-variant hover:bg-surface-container-high relative cursor-pointer"
                title="Notifications"
            >
            ...
            {open && (
                <div
                    className="absolute right-0 top-full mt-1 w-80 max-w-[90vw] bg-surface border border-outline-variant rounded-xl shadow-lg overflow-hidden z-50"
                >
                        ...
                        {notifications.length === 0 ? (
                            ...
                            notifications.slice(0, 5).map((n) => (
                                <button
                                    ...
                                    className={`w-full text-left px-4 py-3 border-b border-outline-variant transition-colors cursor-pointer ${
                                        n.is_read
                                ...
                                >
                                    <div className="flex items-start gap-3">
                                        ...
                                        <div className="flex-1 min-w-0">
                                            <p className={`text-xs font-semibold ${n.is_read ? 'text-on-surface-variant' : 'text-on-surface'}`}>
                                                {n.title}
                                            </p>
                                            <p className="text-xs text-on-surface-variant/70 mt-0.5 line-clamp-2">{n.message}</p>
                                            <p className="text-[10px] text-on-surface-variant/50 mt-1">{timeAgo(n.created_at)}</p>
                                        </div>
                                        {!n.is_read && (
                    ...
                    {notifications.length > 0 && (
                        <div className="px-4 py-2 border-t border-outline-variant bg-surface-container-low flex items-center justify-between">
                            <button
                                onClick={markAllAsRead}
                                disabled={unreadCount === 0}
                                className="text-xs font-semibold text-emerald-700 hover:text-emerald-800 disabled:text-on-surface-variant/50 disabled:cursor-not-allowed transition-colors cursor-pointer flex items-center gap-1"
                            >
