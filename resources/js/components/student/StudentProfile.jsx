...
export default function StudentProfile({ student, onLogout, onNavigate, currentPath, onStudentUpdate }) {
  ...
  const [form, setForm] = useState({ first_name: '', last_name: '', email: '', student_number: '', date_of_birth: '', gender: '', emergency_contact_person: '', emergency_contact_number: '', complete_address: '' });
  ...
  useEffect(() => {
    fetch('/student/api/profile', { credentials: 'same-origin' })
      ...
      .then(data => {
        ...
        setProfile(s);
        setForm({ first_name: s.first_name || '', last_name: s.last_name || '', email: s.email || '', student_number: s.student_number || '', date_of_birth: s.date_of_birth ? s.date_of_birth.slice(0, 10) : '', gender: s.gender || '', emergency_contact_person: s.emergency_contact_person || '', emergency_contact_number: s.emergency_contact_number || '', complete_address: s.complete_address || '' });
        setLoading(false);
      })
  ...
  useEffect(() => {
    ...
    const fadeTimer = setTimeout(() => setMessageFading(true), 2700);
    const clearTimer = setTimeout(() => { setMessage(null); setMessageFading(false); }, 3000);
  ...
  return (
    <StudentDashboardLayout title="My Profile" subtitle="Manage your account" student={student} onLogout={onLogout} onNavigate={onNavigate} currentPath={currentPath}>
      <div className="max-w-container-max mx-auto space-y-6">
          {message && (
            <div className={'flex items-center gap-2 p-3 sm:p-4 rounded-xl border transition-opacity duration-300 ' + (messageFading ? 'opacity-0' : 'opacity-100') + ' ' + (message.type === 'success' ? 'bg-emerald-50 border-emerald-200 text-emerald-800' : 'bg-red-50 border-red-200 text-red-700')}>
          ...
          <div className="bg-surface-container-lowest border border-outline-variant rounded-2xl p-5 sm:p-7 shadow-sm">
            <div className="flex items-center justify-between mb-5">
              <h3 className="text-base sm:text-lg font-bold text-on-surface">Personal Information</h3>
              <button type="button" onClick={openEditModal} className="px-4 py-2 rounded-lg bg-primary text-on-primary font-bold text-label-sm hover:opacity-90 transition-all cursor-pointer">
                Edit
              </button>
            </div>
            ...
            <div className="space-y-4">
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div>
                    <p className="text-label-sm text-on-surface-variant font-medium mb-1">First Name</p>
                    <p className="text-body-md text-on-surface font-semibold">{s?.first_name}</p>
                  </div>
                  <div>
                    <p className="text-label-sm text-on-surface-variant font-medium mb-1">Last Name</p>
                    <p className="text-body-md text-on-surface font-semibold">{s?.last_name}</p>
                  </div>
                </div>
                <div>
                  <p className="text-label-sm text-on-surface-variant font-medium mb-1">Email</p>
                  <p className="text-body-md text-on-surface font-semibold">{s?.email}</p>
                </div>
                <div>
                  <p className="text-label-sm text-on-surface-variant font-medium mb-1">Student Number</p>
                  <p className="text-body-md text-on-surface font-semibold">{s?.student_number}</p>
                </div>
                <div className="pt-4 border-t border-outline-variant">
                  <h4 className="text-base font-bold text-on-surface mb-4">Registrar Information</h4>
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div>
                      <p className="text-label-sm text-on-surface-variant font-medium mb-1">Date of Birth</p>
                      <p className="text-body-md text-on-surface font-semibold">{s?.date_of_birth ? s.date_of_birth.slice(0, 10) : '-'}</p>
                    </div>
                    <div>
                      <p className="text-label-sm text-on-surface-variant font-medium mb-1">Gender</p>
                      <p className="text-body-md text-on-surface font-semibold">{s?.gender || '-'}</p>
                    </div>
                    <div>
                      <p className="text-label-sm text-on-surface-variant font-medium mb-1">Emergency Contact Person</p>
                      <p className="text-body-md text-on-surface font-semibold">{s?.emergency_contact_person || '-'}</p>
                    </div>
                    <div>
                      <p className="text-label-sm text-on-surface-variant font-medium mb-1">Emergency Contact Number</p>
                      <p className="text-body-md text-on-surface font-semibold">{s?.emergency_contact_number || '-'}</p>
                    </div>
                    <div className="sm:col-span-2">
                      <p className="text-label-sm text-on-surface-variant font-medium mb-1">Complete Address</p>
                      <p className="text-body-md text-on-surface font-semibold">{s?.complete_address || '-'}</p>
                    </div>
              </div>
            </div>
          </div>
      ...
      {/* Edit Profile Modal */}
      {showEditModal && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/50 px-3 py-6" onClick={closeEditModal}>
          <div className="w-full max-w-lg max-h-[90vh] overflow-y-auto" onClick={e => e.stopPropagation()}>
            <div className="bg-surface rounded-2xl shadow-sm border border-outline-variant overflow-hidden relative">
              <button type="button" onClick={closeEditModal} className="absolute top-3 right-3 w-11 h-11 flex items-center justify-center text-on-surface-variant hover:text-on-surface hover:bg-surface-container-high transition-colors cursor-pointer z-10 rounded-full">
                <X className="w-4 h-4 sm:w-5 sm:h-5" />
              </button>
              <div className="p-6 sm:p-8">
                <h3 className="text-base sm:text-lg font-bold text-on-surface mb-6">Edit Profile</h3>
                  ...
                  <div className="pt-4 border-t border-outline-variant">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                      <div>
                        ...
                        <input name="date_of_birth" type="date" value={form.date_of_birth} onChange={handleChange} className={inputClass + (errors.date_of_birth ? ' ' + inputErrorClass : '')} />
                        {errors.date_of_birth && <p className="text-body-xs text-error mt-1">{errors.date_of_birth[0]}</p>}
                      </div>
                      <div>
                        <FormSelect
                          label="Gender"
                      ...
                      <div className="sm:col-span-2">
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                          <input name="emergency_contact_person" type="text" value={form.emergency_contact_person} onChange={handleChange} className={inputClass + (errors.emergency_contact_person ? ' ' + inputErrorClass : '')} placeholder="Full name" />
                          <input name="emergency_contact_number" type="tel" value={form.emergency_contact_number} onChange={handleChange} className={inputClass + (errors.emergency_contact_number ? ' ' + inputErrorClass : '')} placeholder="Contact number" />
                        </div>
                        {errors.emergency_contact_person && <p className="text-body-xs text-error mt-1">{errors.emergency_contact_person[0]}</p>}
                        {errors.emergency_contact_number && <p className="text-body-xs text-error mt-1">{errors.emergency_contact_number[0]}</p>}
                      </div>
                      <div className="sm:col-span-2">
                        <label className={labelClass}>Complete Address</label>
                        <textarea name="complete_address" value={form.complete_address} onChange={handleChange} rows="3" className={inputClass + ' resize-none' + (errors.complete_address ? ' ' + inputErrorClass : '')} placeholder="e.g. 123 Mabini St., Brgy. San Jose, Batangas City" />
                        {errors.complete_address && <p className="text-body-xs text-error mt-1">{errors.complete_address[0]}</p>}
                      </div>
                    </div>
                  </div>
                  <div className="flex items-center gap-3 pt-2">
                    <button type="submit" className="px-5 py-2.5 rounded-lg bg-primary text-on-primary font-bold text-label-sm hover:opacity-90 transition-all cursor-pointer">
                      Save Changes
                    </button>
                    <button type="button" onClick={closeEditModal} className="px-5 py-2.5 rounded-lg font-bold text-label-sm text-on-surface-variant bg-surface-container-high hover:bg-surface-container-higher transition-colors cursor-pointer">
                      Cancel
                    </button>
                  </div>
                </form>
      ...
      {showSaveConfirm && (
        ...
        >
          <div
            className="bg-surface rounded-2xl shadow-xl border border-outline-variant w-full max-w-sm p-6"
            onClick={e => e.stopPropagation()}
          >
            <div className="flex items-center justify-between mb-4">
              <h3 className="text-headline-sm font-bold text-on-surface">Confirm Changes</h3>
              <button
                onClick={() => setShowSaveConfirm(false)}
                disabled={saving}
                className="w-8 h-8 flex items-center justify-center text-on-surface-variant hover:text-on-surface hover:bg-surface-container-high transition-colors cursor-pointer rounded-full"
              >
                <X className="w-4 h-4" />
              </button>
            </div>
            ...
            <div className="flex items-center gap-3">
              ...
              <button
                onClick={confirmSave}
                disabled={saving}
                className="flex-1 py-2.5 rounded-lg font-bold transition-all cursor-pointer disabled:opacity-50 text-white bg-primary hover:opacity-90"
              >
