            ...
            <main className="lg:pl-72">
                <header className="sticky top-0 z-10 border-b border-slate-200 bg-white/95 backdrop-blur">
                    <div className="flex min-h-16 items-center justify-between gap-4 px-4 sm:px-6 lg:px-8">
                        <div>
                            <h1 className="text-lg font-semibold text-slate-950 sm:text-xl">Credential Request Management</h1>
                            <p className="text-sm text-slate-500">Backend: Laravel API {backendStatus.toLowerCase()}</p>
                        </div>
                        <div className="flex items-center gap-2">
                            ...
                            <button
                                type="button"
                                className="inline-flex min-h-11 items-center gap-2 rounded-lg bg-teal-700 px-4 text-sm font-semibold text-white shadow-sm hover:bg-teal-800"
                            >
                ...
                <div className="px-4 py-6 sm:px-6 lg:px-8">
                        ...
                        {metrics.map((metric) => (
                            <article key={metric.label} className="rounded-lg border border-slate-200 bg-white p-5 shadow-xs">
                                <div className="flex items-start justify-between gap-4">
                                    ...
                                    <div className="flex size-10 items-center justify-center rounded-lg bg-slate-100 text-teal-700">
                                        <metric.icon aria-hidden="true" className="size-5" />
                                    </div>
                                </div>
                                <p className="mt-4 text-sm text-slate-600">{metric.trend}</p>
                            </article>
                        ))}
                    </section>
                    ...
                    <section className="mt-6 rounded-lg border border-slate-200 bg-white shadow-xs">
                        <div className="flex flex-col gap-4 border-b border-slate-200 p-4 sm:flex-row sm:items-center sm:justify-between">
                            <div>
                                <h2 className="text-base font-semibold text-slate-950">Active credential requests</h2>
                                <p className="text-sm text-slate-500">Review, approve, and track provisioning work.</p>
                            </div>
                            <div className="flex flex-col gap-2 sm:flex-row">
                                ...
                                <button
                                    type="button"
                                    className="inline-flex min-h-11 items-center justify-center gap-2 rounded-lg border border-slate-200 bg-white px-4 text-sm font-medium text-slate-700 hover:bg-slate-100"
                                >
